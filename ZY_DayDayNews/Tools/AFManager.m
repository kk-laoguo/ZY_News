...
+ (AFManager *)sharedUtil {
    ...
    static AFManager * setSharedInstance;
    
    dispatch_once(&onceToken, ^{
        setSharedInstance = [[AFManager alloc] init];
    ...
    switch (method) {
        case METHOD_GET:
        {
            [manager GET:URLString parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
                ...
                succeed([AFManager dictionaryWithJsonString:responseStr]);
                
                KLog(@"\n 请求成功:%@\n\n",[AFManager dictionaryWithJsonString:responseStr]);
            ...
            failure:(void (^)(NSError *))failure
{
    // 0.设置API地址
    URLString = [NSString stringWithFormat:@"%@%@",BASE_URL,[URLString stringByReplacingOccurrencesOfString:@" " withString:@"%20"]];
    KLog(@"\n POST上传单张图片参数列表:%@\n\n%@\n",parameters,[AFManager URLEncryOrDecryString:parameters IsHead:false]);
